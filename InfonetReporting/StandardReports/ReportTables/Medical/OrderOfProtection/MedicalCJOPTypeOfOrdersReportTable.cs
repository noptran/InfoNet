...
namespace Infonet.Reporting.StandardReports.ReportTables.Medical.OrderOfProtection {
	public class MedicalCJOPTypeOfOrdersReportTable : ReportTable<MedicalCJOrderofProtectionLineItem> {
		private double _divisor = 0.0;

		public MedicalCJOPTypeOfOrdersReportTable(string title, int displayOrder) : base(title, displayOrder) { }

		public override void CheckAndApply(MedicalCJOrderofProtectionLineItem item) {
			if (item.IsValidOrder && item.IsNewIssued) {
				_divisor += 1;
				foreach (ReportRow row in Rows)
					if (row.Code == item.TypeOfOPID)
