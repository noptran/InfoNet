...
namespace Infonet.Reporting.StandardReports.ReportTables.ClientInformation.Aggregate {
	public class SubstanceAbuseReportTable : ReportTable<ClientInformationAggregateLineItem> {
		public SubstanceAbuseReportTable (string title, int displayOrder) : base(title, displayOrder) {

		}
		public override void CheckAndApply(ClientInformationAggregateLineItem item) {
			foreach (ReportRow row in Rows) {
				if (row.Code == item.CenterId && item.TypeId == (int)HivMentalSubstanceEnum.Abuse) {
