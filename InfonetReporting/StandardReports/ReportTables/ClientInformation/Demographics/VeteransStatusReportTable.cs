...
namespace Infonet.Reporting.StandardReports.ReportTables.ClientInformation.Demographics {
	public class VeteransStatusReportTable : ReportTable<ClientInformationDemographicsLineItem> {
		public VeteransStatusReportTable(string title, int displayOrder) : base(title, displayOrder) {

		}
		public override void CheckAndApply(ClientInformationDemographicsLineItem item) {
			foreach (ReportRow row in Rows) {
				if (row.Code == item.VetStatusID && item.ClientTypeID == (int)ReportTableSubHeaderEnum.Adult) {
