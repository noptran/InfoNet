...
namespace Infonet.Reporting.StandardReports.ReportTables.Investigation.Medical {
	public class MedicalFacilityTypeReportTable : ReportTable<InvestigationMedicalLineItem> {
		private readonly HashSet<string> _primeIds = new HashSet<string>();

		public MedicalFacilityTypeReportTable(string title, int displayOrder) : base(title, displayOrder) { }

		public override void CheckAndApply(InvestigationMedicalLineItem item) {
			if (!_primeIds.Contains(item.ClientID + "-" + item.MedWhereId))
				foreach (var row in Rows)
					if (row.Code == item.MedWhereId)
						foreach (var header in Headers)
							if (header.Code == item.ClientStatus || header.Code == ReportTableHeaderEnum.Total)
								foreach (var subheader in header.SubHeaders) {
									...
									_primeIds.Add(item.ClientID + "-" + item.MedWhereId);
								}
		}
	}
}