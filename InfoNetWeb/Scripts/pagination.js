...
var Pagination = function (el, options) {
	...
	var $container = $(el),
		...
		firstPageLabel = options.labels.first || '<span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span><span style="margin-left: -6px;" class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span>',
		lastPageLabel = options.labels.last || '<span class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span><span style="margin-left: -6px;" class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span>',
		nextPageLabel = options.labels.next || '<span class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span>',
		previousPageLabel = options.labels.previous || '<span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span>',
	...
	toggleElements = function () {
		...
		$container.find('select').prop('disabled', itemsCount <= 10);
	};
	...
	getRenderedPageRange = function () {
		var result = '<select class="page-range dirtyignore">';
	...
	getRenderedPageNumbers = function () {
		...
		result = '<li data-page="' + FIRST_PAGE_NUMBER + '"><a class="dirtyignore"><span aria-hidden="true">' + firstPageLabel + '</span></a></li>' +
				 '<li data-page="' + PREVIOUS_PAGE_NUMBER + '"><a class="dirtyignore"><span aria-hidden="true">' + previousPageLabel + '</span></a></li>';

		$.each(pageNumbers, function (i, p) {
			result += '<li data-page="' + p + '" ' + (p === currentPage ? 'class="active"' : '') + '><a  class="dirtyignore">' + p + '</a></li>';
		});

		result += '<li data-page="' + NEXT_PAGE_NUMBER + '"><a class="dirtyignore"><span aria-hidden="true">' + nextPageLabel + '</span></a></li>' +
				  '<li data-page="' + LAST_PAGE_NUMBER + '"><a class="dirtyignore"><span aria-hidden="true">' + lastPageLabel + '</span></a></li>';
	...
	disable = function () {
		isDisabled = true;
		$pagesContainer.addClass('disabled', 'dirtyignore');
